  ...
  <div class="card">
    ...
    <div class="otp-inputs">
      <input v-for="(_, i) in 6" :key="i" :ref="el => { if (el) inputs[i] = el as HTMLInputElement }"
        ...
        :class="{ 'otp-error': errorMsg }" @input="onInput(i)" @keydown.backspace="onBackspace(i)" @paste="onPaste" />
    </div>
    ...
    <div class="resend-wrap">
      <span v-if="countdown > 0" class="resend-timer">
        {{ $t('auth.resendTimer', { n: countdown }) }}
      </span>
    ...
    <p class="back-link">
      <NuxtLink to="/register">{{ $t('auth.backToRegister') }}</NuxtLink>
    </p>
  </div>
</template>
...
useHead({ title: 'Velora — Verify Email' })
...
const otpType = ref((route.query.type as string) || 'register')
...
const onInput = (i: number) => {
  ...
  if (val && i < 5) nextTick(() => inputs.value[i + 1]?.focus())
  errorMsg.value = ''
}
...
const handleVerify = async () => {
  ...
  errorMsg.value = ''
  try {
    const res: any = await verifyOtp(email.value, code.value, otpType.value)
...
const handleResend = async () => {
  ...
  try {
    await $fetch(`${config.public.apiBase}/resend-otp`, {
      method: 'POST',
      body: { email: email.value, type: otpType.value },
    })
    countdown.value = 60
    clearInterval(timer)
    timer = setInterval(() => {
    ...
    errorMsg.value = err?.data?.message ?? t('auth.sendFailed')
  } finally {
